...
import type { OracleSnapshot, OracleEvent, ConnectionState, OracleMetrics } from './oracle/types';
import type { RiskDecision, RiskParameters, RiskFactor, RiskAction } from './risk/engine';
import type { ExecutionResult, TradeIntent } from './execution/jupiter';
import type { SignedDecision } from './crypto/signing';
import type { CircuitStatus, CircuitState } from './failsafe/circuitBreaker';
import type { SystemMetrics, DecisionLogEntry, ExecutionLogEntry } from './observability/decisionLog';
...
export class CATEEngine {
  ...
  constructor(config: Partial<CATEEngineConfig> = {}) {
    ...
    console.log('[CATEEngine] Signer public key:', signer.getPublicKey());
  }
  ...
  async initializeChainConfig(
    ...
    const signer = getSigningEngine();
    return onChainTrustService.initializeConfig(signer.getPublicKey());
  }

  /**
   * Publish a decision to chain
   */
  async publishDecisionToChain(
...
export type { OracleSnapshot, OracleMetrics } from './oracle/types';
